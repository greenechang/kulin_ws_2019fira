    ...
    foot_distance_(0.07)
{
  ...
  std::string balance_gain_path;
  std::string joint_feedback_gain_path;
  std::string joint_feedforward_gain_path;
  
  if(ros::param::get("/package_name",package_name)){
    balance_gain_path = ros::package::getPath(package_name) + "/config/balance_gain.yaml";
    joint_feedback_gain_path = ros::package::getPath(package_name) + "/config/joint_feedback_gain.yaml";
    joint_feedforward_gain_path = ros::package::getPath(package_name) + "/config/joint_feedforward_gain.yaml";
  }

  else{
    balance_gain_path = ros::package::getPath("op3_online_walking_module") + "/config/balance_gain.yaml";
    joint_feedback_gain_path = ros::package::getPath("op3_online_walking_module") + "/config/joint_feedback_gain.yaml";
    joint_feedforward_gain_path = ros::package::getPath("op3_online_walking_module") + "/config/joint_feedforward_gain.yaml";
  }
...
void OnlineWalkingModule::parseBalanceGainData(const std::string &path)
{
  ...
  {
    ROS_ERROR("Online: Fail to load yaml file.");
    return;
  }
...
void OnlineWalkingModule::setWholebodyBalanceMsgCallback(const std_msgs::String::ConstPtr& msg)
{
  ...
  std::string package_name;
  ros::param::get("/package_name",package_name);
